...
export default function CandidateStream({ isVideoOn, onMetricsUpdate }) {
  ...
  useEffect(() => {
    ...
    faceMesh.onResults((results) => {
      ...
      const verticalFaceDist = Math.abs(mouthBottom.y - noseTip.y);
      let calculatedScore = Math.min(100, Math.max(50, Math.round(85 + (0.5 - Math.abs(0.5 - noseToEyeRatio)) * 30)));

      setEyeContact(currentEyeContact);
      setHeadPose(currentHeadPose);
      setConfidenceScore(calculatedScore);

      // Optional: Pass per-frame stats back to parent state for post-interview reports
      if (onMetricsUpdate) {
        onMetricsUpdate({
  ...
  return (
    <div className="relative w-full h-full rounded-2xl bg-slate-900 border border-slate-800 flex items-center justify-center overflow-hidden">
      ...
      {/* Candidate Tag */}
      <div className="absolute bottom-3 left-3 bg-slate-950/80 backdrop-blur-md px-3 py-1 rounded-lg border border-slate-800">
